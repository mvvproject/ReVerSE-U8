 		DEVICE	ZXSPECTRUM48
; -----------------------------------------------------------------[19.03.2014]
; U8EP3C Loader Version 0.8 By MVV
; -----------------------------------------------------------------------------
...
SYSTEM_PORT	EQU #0001	; bit2 = 0:Loader ON, 1:Loader OFF; bit1 = 0:SRAM<->CPU0, 1:SRAM<->GS; bit0 = 0:VS1053, 1:M25P40
...
StartProg:
		...
		LD A,%00000001	; Bit2 = 0:Loader ON, 1:Loader OFF; Bit1 = 0:SRAM<->CPU0, 1:SRAM<->GS; Bit0 = 0:VS1053, 1:M25P40
		LD BC,SYSTEM_PORT
		OUT (C),A
...
SPI_LOADER1	CALL SPI_R
		...
		LD A,%00000111	; Bit2 = 0:Loader ON, 1:Loader OFF; Bit1 = 0:SRAM<->CPU0, 1:SRAM<->GS; Bit0 = 0:VS1053, 1:M25P40
		LD BC,SYSTEM_PORT
		OUT (C),A
		LD BC,MASK_PORT
...
SPI_LOADER2	CALL SPI_R
		...
		CALL SPI_END
		LD A,%00000110	; Bit2 = 0:Loader ON, 1:Loader OFF; Bit1 = 0:SRAM<->CPU0, 1:SRAM<->GS; Bit0 = 0:VS1053, 1:M25P40
		LD BC,SYSTEM_PORT
		OUT (C),A
		XOR A
		LD BC,#7FFD
...
STR_TEST1	DB #0D,#0A
		DB #0D,#0A,"ReVerSE-U8 Loader, version 0.8"
